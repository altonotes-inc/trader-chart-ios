//
//  SMA.swift
//  TraderChart
//
//  Created by Keita Yamamoto on 2019/04/22.
//  Copyright © 2019 altonotes Inc. All rights reserved.
//

import UIKit

/// 単純移動平均
open class SMA: TechnicalChart {

    /// 標準の短期線の色
    static let defaultShortColor: UIColor = UIColor(rgbHex: "FFF97D")
    /// 標準の中期線の色
    static let defaultMiddleColor: UIColor = UIColor(rgbHex: "FF7774")
    /// 標準の長期線の色
    static let defaultLongColor: UIColor = UIColor(rgbHex: "8DFAFF")
    
    open var isVisible: Bool = true

    /// 短期線の色
    open var shortColor: UIColor = defaultShortColor
    /// 中期線の色
    open var middleColor: UIColor = defaultMiddleColor
    /// 長期線の色
    open var longColor: UIColor = defaultLongColor

    /// ラインの幅
    open var lineWidth: CGFloat = 1

    /// パラメータ
    open var param = Param()

    /// 直近の計算に使った短期スパン
    public var latestShortSpan: Int?
    /// 直近の計算に使った中期スパン
    public var latestMiddleSpan: Int?
    /// 直近の計算に使った長期スパン
    public var latestLongSpan: Int?

    /// 短期線の計算結果
    open var shortSMA: NumberArray?
    /// 中期線の計算結果
    open var middleSMA: NumberArray?
    /// 長期線の計算結果
    open var longSMA: NumberArray?

    /// ラインチャートの描画
    open var line = LineChartDrawer()

    /// 移動平均の計算
    open var calculator = SMACalculator()

    /// カスタムの凡例
    open var customLegend: ((SMA, Int?) -> Legend?)?

    /// 標準の凡例
    open var defaultLegend: Legend? {
        return Legend([
            ColorText("移動平均:\(param.shortSpan)", shortColor, visible: param.shortOn),
            ColorText("移動平均:\(param.middleSpan)", middleColor, visible: param.middleOn),
            ColorText("移動平均:\(param.longSpan)", longColor, visible: param.longOn),
        ])
    }

    public func legend(selectedIndex: Int?) -> Legend? {
        if let customLegend = customLegend {
            return customLegend(self, selectedIndex)
        }
        return defaultLegend
    }

    /// 初期化
    public init() {}
    
    public func reflectColorConfig(_ colorConfig: ColorConfig) {
        shortColor = colorConfig["sma.short"] ?? SMA.defaultShortColor
        middleColor = colorConfig["sma.middle"] ?? SMA.defaultMiddleColor
        longColor = colorConfig["sma.long"] ?? SMA.defaultLongColor
    }

    public func updateData(_ data: ChartData?, updatedFrom: Int? = nil) {
        if param.shortOn {
            shortSMA?.removeLastFrom(updatedFrom)
            shortSMA = calculator.calculate(span: param.shortSpan, src: data?.closeList, existingSMA: shortSMA)
            latestShortSpan = param.shortSpan
        }
        if param.middleOn {
            middleSMA?.removeLastFrom(updatedFrom)
            middleSMA = calculator.calculate(span: param.middleSpan, src: data?.closeList, existingSMA: middleSMA)
            latestMiddleSpan = param.middleSpan
        }
        if param.longOn {
            longSMA?.removeLastFrom(updatedFrom)
            longSMA = calculator.calculate(span: param.longSpan, src: data?.closeList, existingSMA: longSMA)
            latestLongSpan = param.longSpan
        }
    }

    public func removeOldData(count: Int) {
        shortSMA?.removeFirst(count)
        middleSMA?.removeFirst(count)
        longSMA?.removeFirst(count)
    }

    public func onParameterChanged() {
        if param.shortSpan != latestShortSpan {
            shortSMA = nil
        }
        if param.middleSpan != latestMiddleSpan {
            middleSMA = nil
        }
        if param.longSpan != latestLongSpan {
            longSMA = nil
        }
    }

    public func updateXAxis(_ xAxis: XAxis, data: ChartData?) {}

    public func updateYAxis(_ yAxis: YAxis, data: ChartData?, height: CGFloat, visibleSpan: ClosedRange<Int>) {
        if param.shortOn {
            yAxis.updateRangeAll(shortSMA, span: visibleSpan)
        }

        if param.middleOn {
            yAxis.updateRangeAll(middleSMA, span: visibleSpan)
        }

        if param.longOn {
            yAxis.updateRangeAll(longSMA, span: visibleSpan)
        }
    }
    
    public func draw(context: ChartDrawingContext, yAxis: YAxis, data: ChartData?, rect: CGRect, visibleSpan: ClosedRange<Int>) {
        if param.shortOn {
            line.draw(context: context,
                      rect: rect,
                      yAxis: yAxis,
                      visibleSpan: visibleSpan,
                      data: shortSMA,
                      lineWidth: lineWidth,
                      color: shortColor)
        }

        if param.middleOn {
            line.draw(context: context,
                      rect: rect,
                      yAxis: yAxis,
                      visibleSpan: visibleSpan,
                      data: middleSMA,
                      lineWidth: lineWidth,
                      color: middleColor)
        }

        if param.longOn {
            line.draw(context: context,
                      rect: rect,
                      yAxis: yAxis,
                      visibleSpan: visibleSpan,
                      data: longSMA,
                      lineWidth: lineWidth,
                      color: longColor)
        }
    }

    public func clear() {
        shortSMA = nil
        middleSMA = nil
        longSMA = nil
    }

    /// 移動平均のパラメータ
    public class Param {
        public var shortOn = true
        public var middleOn = true
        public var longOn = true
        public var shortSpan = 5
        public var middleSpan = 25
        public var longSpan = 75
        
        public init() {}
    }
}
